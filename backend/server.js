...
// Optional local .env loader without adding any dependency.
// Render uses Environment Variables directly, so .env is only for local/dev fallback.
function loadDotEnv() {
  const candidates = [path.join(ROOT, '.env'), path.join(path.dirname(ROOT), '.env')];
...
function envBool(name) {
  ...
  return !/^false|0|no$/i.test(String(v));
}
function envConfig() {
  const out = {};
  // Accept both Render-style camelCase names and production-style uppercase aliases.
  const googleKey = envFirst('GOOGLE_API_KEY', 'GOOGLE_MAPS_API_KEY', 'MAPS_API_KEY', 'googleApiKey', 'googleMapsApiKey', 'mapsApiKey');
  const barikoiKey = envFirst('BARIKOI_API_KEY', 'BARIKOI_KEY', 'barikoiApiKey', 'barikoiKey');
  if (googleKey) out.googleApiKey = googleKey;
  if (barikoiKey) out.barikoiApiKey = barikoiKey;
  const googleLimit = envFirst('GOOGLE_DAILY_LIMIT', 'googleDailySafetyLimit', 'GOOGLE_SAFETY_LIMIT');
  const barikoiLimit = envFirst('BARIKOI_DAILY_LIMIT', 'barikoiDailySafetyLimit', 'BARIKOI_SAFETY_LIMIT');
  if (googleLimit) out.googleDailySafetyLimit = Number(googleLimit);
  if (barikoiLimit) out.barikoiDailySafetyLimit = Number(barikoiLimit);
  const fg = envBool('FETCH_GOOGLE'); if (fg !== undefined) out.fetchGoogle = fg;
  const fb = envBool('FETCH_BARIKOI'); if (fb !== undefined) out.fetchBarikoi = fb;
  const fo = envBool('FETCH_OSM'); if (fo !== undefined) out.fetchOsm = fo;
  return out;
}
function config() {
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (url.pathname === '/api/settings' && req.method === 'GET') { const c = config(); const e = envConfig(); return send(res, 200, { ...c, googleApiKey:maskKey(c.googleApiKey), barikoiApiKey:maskKey(c.barikoiApiKey), keyStatus:{ googleConfigured:!!c.googleApiKey, barikoiConfigured:!!c.barikoiApiKey, googleFromEnv:!!e.googleApiKey, barikoiFromEnv:!!e.barikoiApiKey }, storage:{ dir:STORAGE, persistent:/^\/var\/data/.test(STORAGE) || !!process.env.DATA_DIR || !!process.env.MAP_ZONE_DATA_DIR || !!process.env.RENDER_DISK_PATH } }); }
    if (url.pathname === '/api/settings' && req.method === 'POST') {
      const body = await readBody(req);
      const old = readJson(CONFIG_FILE, {});
      const env = envConfig();
      const next = { ...old, ...body };
      // Do not let blank/masked dashboard inputs delete working environment keys.
      const badGoogle = !body.googleApiKey || /^\*|.*\.\.\./.test(String(body.googleApiKey));
      const badBarikoi = !body.barikoiApiKey || /^\*|.*\.\.\./.test(String(body.barikoiApiKey));
      if (badGoogle) { if (old.googleApiKey) next.googleApiKey = old.googleApiKey; else delete next.googleApiKey; }
      if (badBarikoi) { if (old.barikoiApiKey) next.barikoiApiKey = old.barikoiApiKey; else delete next.barikoiApiKey; }
      // Persist only non-secret settings when keys are already in env.
      if (env.googleApiKey) delete next.googleApiKey;
      if (env.barikoiApiKey) delete next.barikoiApiKey;
      writeJson(CONFIG_FILE, next);
      const merged = config();
      return send(res, 200, { ok:true, settings:{...next, googleApiKey:maskKey(merged.googleApiKey), barikoiApiKey:maskKey(merged.barikoiApiKey), keyStatus:{ googleConfigured:!!merged.googleApiKey, barikoiConfigured:!!merged.barikoiApiKey, googleFromEnv:!!env.googleApiKey, barikoiFromEnv:!!env.barikoiApiKey }, storage:{ dir:STORAGE, persistent:/^\/var\/data/.test(STORAGE) || !!process.env.DATA_DIR || !!process.env.MAP_ZONE_DATA_DIR || !!process.env.RENDER_DISK_PATH } } });
    }
